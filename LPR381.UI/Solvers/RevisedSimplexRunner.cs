    ...
    public sealed class RevisedSimplexRunner(bool isPrimal) : SolverRunner
    {
                ...
                if (FSharpInterop.TrySome(provider.SimplexResult, out var res))
                {
                    ...
                    switch (caseName)
                    {
                        case "Optimal":
                            ...
                            var objTypeStr = canon.ObjectiveType == ObjectiveType.Max ? "maximization" : "minimization";
                            summary.Message = $"Optimal solution found using Revised Primal Simplex method for {objTypeStr} problem.";
                            break;
                        case "Unbounded":
        ...
        protected override SolveSummary Solve(LPFormulation model)
        {
            ...
            if(isPrimal) {
                ...
                // For dual simplex, we need to detect phase transitions
                phasePrefix = "";
            }

            return isPrimal ? Solve(phasePrefix, root) : SolveDualWithPhaseTransition(root, model);
        }
        
        private SolveSummary SolveDualWithPhaseTransition(ITree<RevisedSimplexNode> root, LPFormulation model)
        {
            var summary = new SolveSummary();
            var stack = new Stack<(ITree<RevisedSimplexNode> node, int idx, bool isDualPhase)>();
            
            // Start in dual phase
            stack.Push((root, 0, true));
            int dualIterations = 0;
            int primalIterations = 0;

            while (stack.Count > 0)
            {
                var (cur, idx, isDualPhase) = stack.Pop();
                var node = cur.Item;
                
                // Check if we should transition from dual to primal phase
                bool currentlyInDualPhase = isDualPhase;
                if (isDualPhase)
                {
                    // Check if RHS is now feasible (all non-negative)
                    var rhs = FSharpInterop.MatrixVectorMultiply(node.BInverse, node.Canon.RHS.ToArray());
                    ...
                    currentlyInDualPhase = !allNonNegative;
                }

                var (stateCase, stateFields) = FSharpInterop.ReadUnion(node.State);
                string phaseLabel = currentlyInDualPhase ? "Dual" : "Primal";
                int phaseIdx = currentlyInDualPhase ? dualIterations : primalIterations;
                string title = $"{phaseLabel} Iteration {phaseIdx}";
                ...
                if (stateCase == "Pivot")
                {
                    var leavingBasis = (int)stateFields[0];
                    var enteringVariable = (int)stateFields[1];
                    var enteringVariableName = node.Canon.VariableNames[enteringVariable];
                    var leavingVariableName = node.Canon.VariableNames[node.Basis[leavingBasis]];
                    var eta = (EtaMatrix)stateFields[2];
                    ...
                    try
                    {
                        ...
                        if (priceCase == "Primal")
                        {
                            ...
                            _iterations.Add(new IterationTableau
                            {
                                Title = $"{title} – Primal Price Out Info (decision: pivot from {leavingVariableName} to {enteringVariableName})",
                                Columns = columns.ToArray(),
                                Rows = rows.ToArray(),
                                Values = values
                            });
                            ...
                            var columns = new List<string>();
                            var rows = new[] { "z", $"c{leavingBasis+1}", "absRatio" };
                            ...
                            for(int i = 0; i < leavingRow.Length; i++) {
                                ...
                                columns.Add(node.Canon.VariableNames[i]);
                            }
                            columnValues.Add(new double[] {
                                node.ObjectiveValue,
                                rhs[leavingBasis],
                ...
                if (FSharpInterop.TrySome(provider.SimplexResult, out var res))
                {
                    ...
                    switch (caseName)
                    {
                        case "Optimal":
                            ...
                            summary.Objective = (double)fields[2];
                            summary.VariableValues = FSharpInterop.ToDict(fields[1]);
                            summary.Message = $"Optimal solution found using Revised Dual Simplex method for {objTypeStr} problem. Completed {dualIterations} dual iterations and {primalIterations} primal iterations.";
                            break;
                        case "Unbounded":
                            summary.Message = $"Unbounded (variable {fields[0]}).";
                            break;
                ...
                var children = cur.Children;
                for (int i = children.Length - 1; i >= 0; --i)
                    stack.Push((children[i], idx + 1, !currentlyInDualPhase));
            }
